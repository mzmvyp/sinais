# analyzer_corrigido.py - CORREÇÕES PARA OS PROBLEMAS IDENTIFICADOS

"""
CORREÇÕES APLICADAS:
1. ✅ Sistema rigoroso de qualidade - fallback corrigido
2. ✅ Candlesticks com filtro ajustado (0.75 em vez de 0.90)
3. ✅ Logs detalhados reativados
4. ✅ Lógica de 1 sinal por crypto verificada
5. ✅ Target1 -> Stop substituição implementada
"""
...
# 🚨 CORREÇÃO 1: Import com fallback mais robusto
try:
    ...
    print("✅ Sistema rigoroso de qualidade CARREGADO")
except ImportError as e:
    RIGOROUS_QUALITY_AVAILABLE = False
    print(f"⚠️ Sistema rigoroso não disponível: {e}")
    print("💡 Usando sistema padrão de qualidade com melhorias")

try:
    from config.volume_validation_config import create_enhanced_volume_validator
    ENHANCED_VOLUME_AVAILABLE = True
    print("✅ Sistema aprimorado de volume CARREGADO")
except ImportError as e:
    ENHANCED_VOLUME_AVAILABLE = False
    print(f"⚠️ Volume aprimorado não disponível: {e}")
...
class SignalConflictResolver:
    ...
    def resolve_conflicts(self, signals: List[EnhancedTradingSignal]) -> List[EnhancedTradingSignal]:
        ...
        for symbol, group_signals in grouped.items():
            if len(group_signals) == 1:
                ...
                # 🚨 CORREÇÃO 3: Log detalhado para sinal único
                signal = group_signals[0]
                self.logger.info(f"✅ SINAL ÚNICO: {symbol} | {signal.timeframe} | {signal.detector_name} | {signal.signal_type} | Conf: {signal.confidence:.3f}")
            else:
                # CONFLITO DETECTADO
                conflicts_resolved += 1
                self.logger.warning(f"🚨 CONFLITO: {symbol} - {len(group_signals)} sinais")
                
                # Log detalhado dos sinais em conflito
                for i, s in enumerate(group_signals):
                    self.logger.info(f"   {i+1}. {s.timeframe} | {s.detector_name} | {s.signal_type} | Conf: {s.confidence:.3f}")
    ...
    def _select_best_signal_same_timeframe(self, signals: List[EnhancedTradingSignal]) -> EnhancedTradingSignal:
        ...
        detector_priority = {
            ...
            'Double_Top': 6,
            'Double_Bottom': 6,
            'Bullish_Engulfing': 8,  # 🚨 PRIORIDADE AUMENTADA
            'Bearish_Engulfing': 8,  # 🚨 PRIORIDADE AUMENTADA
            'Hammer': 5,
            'Shooting_Star': 5
        }
    ...
    def _select_best_signal(self, signals: List[EnhancedTradingSignal]) -> EnhancedTradingSignal:
        ...
        detector_priority = {
            'technical': 10,
            'pattern': 5,
            'candlestick': 7  # 🚨 PRIORIDADE AUMENTADA
        }
...
class MultiTimeframeAnalyzer:
    def __init__(self):
        ...
        # 🚨 CORREÇÃO 1: Sistema rigoroso com fallback melhorado
        if RIGOROUS_QUALITY_AVAILABLE:
            ...
            # Fallback MELHORADO para sistema padrão
            self.conflict_resolver = SignalConflictResolver()
            self.quality_mode = "standard_enhanced"  # 🚨 NOVO
            self.logger.info("⚠️ Usando sistema padrão MELHORADO de qualidade")

        # 🚨 CORREÇÃO 3: Sistema aprimorado de volume
        if ENHANCED_VOLUME_AVAILABLE:
            self._validate_with_volume_enhanced = create_enhanced_volume_validator()
            self.volume_validation_type = "enhanced"
            self.logger.info("🔊 Sistema APRIMORADO de validação de volume ativado")
        else:
            self.volume_validation_type = "standard"
            self.logger.info("⚠️ Usando validação de volume padrão")
        ...
        self.logger.info("🔧 MultiTimeframeAnalyzer CORRIGIDO inicializado:")
        self.logger.info(f"  • Timeframes ativos: {enabled_timeframes}")
        self.logger.info(f"  • Qualidade: {self.quality_mode.upper()}")
        self.logger.info(f"  • Volume: {self.volume_validation_type.upper()}")
    ...
    def analyze_symbol_all_timeframes(self, symbol: str) -> Dict[str, Any]:
        """Análise CORRIGIDA com sistema melhorado"""
        
        # 🚨 CORREÇÃO 4: Verificação melhorada de sinais ativos
        active_signals_count = self.signal_writer.get_active_signals_count(symbol)
        if active_signals_count > 0:
            self.logger.info(f"🚫 ANÁLISE BLOQUEADA para {symbol}: {active_signals_count} sinal(s) ativo(s)")
            return {
                'symbol': symbol, 
                'status': 'blocked', 
                'reason': f'{active_signals_count}_active_signals',
                'signals_detected': 0, 
                'signals_validated': 0, 
                'signals_saved': 0
            }
        ...
        for tf in enabled_timeframes:
            try:
                ...
                self.logger.debug(f"📊 Dados {symbol} {tf}: {len(market_data.data) if market_data else 0} registros")
        ...
        for timeframe in enabled_timeframes:
            ...
            if market_data and market_data.is_sufficient_data:
                try:
                    tf_result = self._analyze_single_timeframe_corrected(symbol, timeframe, market_data)
                    tf_signals = tf_result.get('signals', [])
                    all_signals.extend(tf_signals)
                    
                    self.logger.info(f"📈 Timeframe {timeframe}: {len(tf_signals)} sinais para {symbol}")
                            
                except Exception as e:
                    self.logger.error(f"❌ Erro na análise de {symbol} em {timeframe}: {e}")
            else:
                self.logger.warning(f"⚠️ Análise para {symbol} em {timeframe} pulada (dados insuficientes).")
        ...
        if self.quality_mode == "rigorous" and hasattr(self, 'enhanced_signal_processing'):
            ...
            # Sistema padrão MELHORADO: resolve conflitos com preferência 5m
            if len(all_signals) > 1:
                self.logger.info(f"⚖️ Resolvendo conflitos em {len(all_signals)} sinais para {symbol}")
                filtered_signals = self.conflict_resolver.resolve_conflicts(all_signals)
                elimination_details = {'reason': 'standard_enhanced_conflict_resolution'}
                self.logger.info(f"✅ Após resolução: {len(filtered_signals)} sinais para {symbol}")
            else:
                filtered_signals = all_signals
                elimination_details = {'reason': 'no_conflicts'}

        # Validação inteligente MELHORADA
        validated_signals = self._intelligent_signal_validation_enhanced(filtered_signals, market_data_by_tf)
        ...
        if validated_signals:
            ...
            try:
                if self.signal_writer.write_enhanced_signal(signal):
                    ...
                    # 🚨 CORREÇÃO 3: Log detalhado do sinal salvo
                    ...
                    # 🚨 CORREÇÃO 5: Log dos targets (2 targets apenas)
                    targets_info = f"T1: {signal.targets[0]:.4f}, T2: {signal.targets[1]:.4f}" if len(signal.targets) >= 2 else "Targets: N/A"
                    
                    self.logger.info(
                        f"🎯 SINAL SALVO: {symbol} | {signal.timeframe} | {signal.detector_name} | "
                        f"{signal.signal_type} | Entry: {signal.entry_price:.4f} | "
                        f"Stop: {signal.stop_loss:.4f} | {targets_info} | "
                        f"Conf: {signal.confidence:.3f}{quality_info}"
                    )
            except Exception as e:
    ...
    def _analyze_single_timeframe_corrected(self, symbol: str, timeframe: str, market_data: MarketData) -> Dict[str, Any]:
        """Análise corrigida de um timeframe - CANDLESTICKS COM FILTRO AJUSTADO"""
        
        # PROTEÇÃO: Verifica se o timeframe é permitido
        if timeframe not in ["5m", "15m"]:
            self.logger.warning(f"⚠️ Timeframe {timeframe} não permitido para {symbol}, ignorando...")
            return {'signals': []}
        ...
        def create_valid_signal(**kwargs):
            try:
                ...
                self.logger.info(f"✅ Sinal criado: {timeframe} {signal.detector_name} | {signal.signal_type} | {signal.confidence:.3f}")
            except Exception as e:
                self.logger.warning(f"⚠️ Sinal para {symbol} em {timeframe} descartado: {e}")
        ...
        if 'technical' in tf_config.enabled_detectors:
            try:
                ...
                self.logger.info(f"🔧 Sinais técnicos {timeframe} para {symbol}: {len(raw_signals)}")
            except Exception as e:
        ...
        if 'patterns' in tf_config.enabled_detectors and PATTERNS_AVAILABLE:
            try:
                ...
                for s in raw_signals:
                     create_valid_signal(**s.__dict__)
                self.logger.info(f"📊 Sinais de padrões {timeframe} para {symbol}: {len(raw_signals)}")
            except Exception as e:
                self.logger.error(f"❌ Erro na análise de padrões para {symbol} {timeframe}: {e}")

        # 🚨 CORREÇÃO 2: ANÁLISE DE CANDLESTICK COM FILTRO AJUSTADO
        if 'candlestick' in tf_config.enabled_detectors and CANDLESTICK_AVAILABLE:
            try:
                ...
                if not df_for_cs.empty:
                    # 1. Gera sinais de candlestick
                    cs_signals_raw = generate_candlestick_signals(df_for_cs, symbol)
                    
                    # 🚨 CORREÇÃO 2: Filtro AJUSTADO (0.75 em vez de 0.90)
                    high_quality_cs = [cs for cs in cs_signals_raw if cs.get('confidence', 0) >= 0.75]
                    
                    for cs in high_quality_cs[:2]:  # Máximo 2 sinais de candlestick
                        create_valid_signal(**cs)
                    
                    self.logger.info(f"🕯️ Candlesticks {timeframe} para {symbol}: {len(cs_signals_raw)} total → {len(high_quality_cs)} filtrados (conf >= 0.75)")
                    
                    # Backup completo se sistema rigoroso disponível
    ...
    def _intelligent_signal_validation_enhanced(self, signals: List[EnhancedTradingSignal], market_data_by_tf: Dict) -> List[EnhancedTradingSignal]:
        """Sistema de validação MELHORADO com logs detalhados"""
        if not signals:
            return []
        ...
        for signal in signals:
            ...
            self.logger.info(f"🔍 VALIDANDO: {signal.symbol} | {signal.timeframe} | {signal.detector_name} | Conf: {signal.confidence:.3f}")
            ...
            if microstructure_available:
                try:
                    ...
                    if is_micro_valid:
                        ...
                        self.logger.debug(f"   ✅ Microestrutura: {micro_note}")
                    else:
                        validation_notes.append(f"⚠️  Micro: {micro_note}")
                        self.logger.debug(f"   ⚠️ Microestrutura: {micro_note}")
                except Exception as e:
            ...
            if not primary_validation_passed:
                try:
                    ...
                    if is_momentum_valid:
                        ...
                        self.logger.debug(f"   ✅ Momentum técnico: {momentum_note}")
                    else:
                        validation_notes.append(f"❌ Momentum: {momentum_note}")
                        self.logger.debug(f"   ❌ Momentum técnico: {momentum_note}")
                except Exception as e:
            ...
            # 2. VALIDAÇÃO DE VOLUME (aprimorada se disponível)
            max_score += 2
            try:
            ...
            if self.quality_mode == "rigorous":
                ...
                # 🚨 CORREÇÃO: Confidence mais relaxada para sistema padrão
                min_confidence = 0.72  # Reduzido de 0.75 para 0.72
            
            if signal.confidence >= min_confidence:
                validation_score += 1
                validation_notes.append(f"✅ Conf: {signal.confidence:.3f} >= {min_confidence:.3f}")
                self.logger.debug(f"   ✅ Confiança: {signal.confidence:.3f} >= {min_confidence:.3f}")
            else:
                validation_notes.append(f"⚠️  Conf: {signal.confidence:.3f} < {min_confidence:.3f}")
                self.logger.debug(f"   ⚠️ Confiança: {signal.confidence:.3f} < {min_confidence:.3f}")

            # DECISÃO: Critérios ajustados para sistema padrão
            success_rate = validation_score / max_score
            
            # Critérios de validação por sistema
            if self.quality_mode == "rigorous":
                if signal.timeframe == "5m":
                ...
                # 🚨 CORREÇÃO: Sistema padrão com critérios mais relaxados
                if signal.timeframe == "5m":
                    required_rate = 0.45  # Reduzido de 0.50 para 0.45
                elif microstructure_available:
                    required_rate = 0.55  # Reduzido de 0.60 para 0.55
                else:
                    required_rate = 0.50  # Reduzido de 0.55 para 0.50
            ...
            if success_rate >= required_rate:
                ...
                signal.market_conditions['quality_mode'] = self.quality_mode
                validated_signals.append(signal)
                
                self.logger.info(f"✅ SINAL APROVADO: {signal.symbol} | {signal.timeframe} | {signal.detector_name} | Score: {validation_score}/{max_score} ({success_rate:.1%}) [{self.quality_mode}]")
            else:
    ...
    # Métodos auxiliares (mantidos iguais, só adicionando logs)
    def _check_microstructure_availability_with_timeout(self) -> bool:
        ...
        try:
            ...
            self._microstructure_available = (
                ...
                test_result.get('sample_data_count', 0) > 20
            )
    ...
    def run_continuous_multi_timeframe_analysis(self, base_interval: int = None):
        """Execução contínua CORRIGIDA com logs detalhados"""
        if base_interval is None: 
           base_interval = settings.system.analysis_interval
        
        self.logger.info("🚀 Iniciando análise contínua CORRIGIDA")
        ...
        while True:
            try:
                ...
                for i, symbol in enumerate(valid_symbols, 1):
                    try:
                        ...
                        if result.get('status') == 'blocked':
                            ...
                            self.logger.info(f"🚫 {symbol} ({i}/{len(valid_symbols)}): BLOQUEADO ({reason}) em {symbol_time:.1f}s")
                ...
                self.logger.info(f"✅ === CICLO {cycle_count} CONCLUÍDO === | {total_signals} novos | {blocked_count} bloqueados | {error_count} erros | {cycle_time:.1f}s")
            ...
            self.logger.info(f"⏳ Aguardando próximo ciclo em {base_interval}s...")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            components = {
                ...
                'candlestick_analyzer': 'OK_FIXED_FILTER' if CANDLESTICK_AVAILABLE else 'NOT_AVAILABLE',  # 🚨 CORRIGIDO
                'microstructure_validation': 'OK' if microstructure_status else 'FALLBACK_MODE',
                'quality_system': self.quality_mode.upper(),  # 🚨 CORRIGIDO
                'volume_validation': self.volume_validation_type.upper(),  # 🚨 NOVO
                'signal_backup_system': 'COMPREHENSIVE' if RIGOROUS_QUALITY_AVAILABLE else 'BASIC',
                'single_signal_control': 'ACTIVE',
                'timeframe_priority': '5M_ABSOLUTE_15M_FALLBACK',
                'anti_hang_protection': 'ACTIVE',
                'logging_level': 'DETAILED_INFO'  # 🚨 NOVO
            }
            
            return {
                'status': 'OK',
                'system_type': 'Trading Analyzer - CORRIGIDO v2.1.1',  # 🚨 VERSÃO CORRIGIDA
                'timestamp': datetime.now().isoformat(),
                'components': components,
                'symbols_available': len(symbols),
                'enabled_timeframes': enabled_timeframes,
                'priority_logic': '5m absolute priority, 15m only when no 5m available',
                'microstructure_available': microstructure_status,
                'active_signals_sample': total_active_signals,
                'fixes_applied': {  # 🚨 NOVO
                    'rigorous_quality_fallback': 'FIXED',
                    'candlestick_filter_threshold': 'REDUCED_TO_0.75',
                    'logging_level': 'ENHANCED_INFO',
                    'volume_validation': 'ENHANCED_IF_AVAILABLE',
                    'signal_validation_criteria': 'RELAXED_FOR_STANDARD_MODE'
                },
                'configuration': {
                    'multi_timeframe_enabled': True,
                    'timeframes_active': enabled_timeframes,
                    'single_signal_per_crypto': True,
                    'timeframe_priority_logic': '5m_absolute_15m_fallback',
                    'patterns_simplified': True,
                    'automatic_cleanup': settings.system.auto_cleanup_enabled,
                    'technical_stop_loss': True,
                    'rigorous_quality_control': self.quality_mode == 'rigorous',
                    'comprehensive_backup': RIGOROUS_QUALITY_AVAILABLE,
                    'anti_hang_protection': True,
                    'validation_timeout': True,
                    'candlestick_confidence_threshold': 0.75  # 🚨 CORRIGIDO
                }
            }
        except Exception as e:
            return {
